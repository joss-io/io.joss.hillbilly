...
import com.jive.ftw.sip.parameters.api.Parameters;
...
public class ApiUtils
{
  ...
  public static final Address convert(final NameAddr addr)
  {
    ...
    return new Address(
        ...
        addr.getParameters().or(DefaultParameters.EMPTY).toString());
  }
  ...
  public static Request convert(SipRequest req)
  {
    ...
    return new Request(convert(req.getUri()), headers, req.getBody());
  }
  ...
  {
    return new SipReason(reason.getProtocol().toString(), reason.getCause().get(), reason.getText().orNull());
  }
